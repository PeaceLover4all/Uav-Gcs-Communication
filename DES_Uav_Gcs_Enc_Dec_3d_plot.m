...
key_sizes = [64 128]; % DES key size is either 64 or 128 bits
...
for i = 1:length(key_sizes)
    ...
    for j = 1:length(message_sizes)
        ...
        % Generate random key
        key = uint8(randi([0, 255], 1, key_size/8));
        ...
        telemetry_data_int8 = int8(telemetry_data_int);

        % Start timer for encryption
        tic

        % Perform DES encryption
        ciphertext = des_encrypt(telemetry_data_int8, key);
...
ylabel('key sizes [64 128] (bits)');
zlabel('Encryption Time (seconds)');
title('DES Average Encryption Times');
...
title('DES Average Decryption Times');
...
function ciphertext = des_encrypt(message, key)
    % Create a Java DES cipher
    cipher = javax.crypto.Cipher.getInstance('DES');
    
    % Create a Java secret key
    secret_key = javax.crypto.spec.SecretKeySpec(key(1:8), 'DES');
...
function plaintext = des_decrypt(ciphertext, key)
    % Create a Java DES cipher
    cipher = javax.crypto.Cipher.getInstance('DES');
