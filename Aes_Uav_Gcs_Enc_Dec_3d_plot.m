...
for i = 1:length(key_sizes)
    ...
    for j = 1:length(message_sizes)
        ...
        ciphertext = aes_encrypt(telemetry_data_int8, key);
...
title('AES Average Encryption Times');
...
title('AES Average Decryption Times');
...
function ciphertext = aes_encrypt(message, key)
    % Create a Java AES cipher
    cipher = javax.crypto.Cipher.getInstance('AES');
    
    % Create a Java secret key
    secret_key = javax.crypto.spec.SecretKeySpec(key, 'AES');
...
function decrypted_message = aes_decrypt(ciphertext, key)
    % Create a Java AES cipher
    cipher = javax.crypto.Cipher.getInstance('AES');
    ...
    decrypted_message = cipher.doFinal(ciphertext);
end